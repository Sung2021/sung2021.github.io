...
# Filtering by UMI & mitocondrial content ratio {.tabset} 


## UMI distribution  

(This code shows the UMI distribution by treatments)
```{r}
obj.srt@meta.data %>%
  ggplot(aes(x = treatment, y = nCount_RNA)) +
  geom_boxplot() +
...
(This code shows the Mitochondrial content distribution by treatments)
```{r}
# Add mitochondrial content (human: "-MT")
obj.srt[["percent.mt"]] <- PercentageFeatureSet(obj.srt, pattern = "^MT-")


obj.srt@meta.data %>%
  ggplot(aes(x = treatment, y = percent.mt)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, size = 0.2, alpha = 0.1) +
  scale_y_sqrt()
```
...
# Number of cells with selected cutoffs {.tabset} 

## nCount_RNA >= 500 & percent.mt <= 25  
```{r}
obj.srt@meta.data %>% filter(predicted_doublet == "False") %>% 
  filter(nCount_RNA >= 500 & percent.mt <= 25) %>% 
  select(treatment) %>% table() %>% data.frame() %>% 
  DT::datatable(colnames = c("sample", "Number of cells"), 
                caption = "nCount_RNA >= 500 & percent.mt <= 25")
```

## nCount_RNA >= 1000 & percent.mt <= 25  
```{r}
obj.srt@meta.data %>% filter(predicted_doublet == "False") %>% 
  filter(nCount_RNA >= 1000 & percent.mt <= 25) %>% 
  select(treatment) %>% table() %>% data.frame() %>% 
  DT::datatable(colnames = c("sample", "Number of cells"), 
                caption = "nCount_RNA >= 1000 & percent.mt <= 25")
```
...
```{r}
obj.srt@meta.data %>% filter(predicted_doublet == "False") %>% 
  filter(nCount_RNA >= 500 & percent.mt <= 10) %>% 
  select(treatment) %>% table() %>% data.frame() %>% 
  DT::datatable(colnames = c("sample", "Number of cells"), 
                caption = "nCount_RNA >= 500 & percent.mt <= 10")
```
...
```{r}
obj.srt@meta.data %>% filter(predicted_doublet == "False") %>% 
  filter(nCount_RNA >= 1000 & percent.mt <= 10) %>% 
  select(treatment) %>% table() %>% data.frame() %>% 
  DT::datatable(colnames = c("sample", "Number of cells"), 
                caption = "nCount_RNA >= 1000 & percent.mt <= 10")
```
<br><br>
...
* nCount_RNA >= 1000
* percent.mt <= 10
* no doublets (predicted_doublet == "False")
...
DimPlot(obj.srt, group.by = 'treatment', alpha = 0.8) +
  scale_color_paletteer_d("ggsci::nrc_npg") + theme_bw()
```

## Split by sample  
```{r, fig.width=11, fig.height=9}
DimPlot(obj.srt, group.by = 'treatment', 
        split.by = 'treatment', ncol = 2, alpha = 0.5) +
  scale_color_paletteer_d("ggsci::nrc_npg") + theme_bw()
```
...
# If you want the table with numbers, use the code below
obj.srt@meta.data %>% select(treatment) %>% 
  table() %>% data.frame() %>% 
  DT::datatable(caption = 'Number of Cells by sample')

# Color palette for the plot 
orig.ident_fils = scale_fill_paletteer_d("ggsci::nrc_npg") 

# If you like the table as a barplot, use the code below
obj.srt@meta.data %>% ggplot(aes(treatment, fill=treatment)) + 
  geom_bar(alpha=0.7, color="grey5", size=0.1) +
  geom_text(stat="count", aes(label= ..count..), vjust=-0.5, size=3) +
  orig.ident_fils + xlab("") + 
  theme_classic() +
...
Two default clustering resolutions are provided, but the optimal resolution for downstream analysis will depend on the specific goals and requirements of each individual project.   
<br>
