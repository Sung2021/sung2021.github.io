...
subtitle: "DESeq2 · GSEA · ssGSEA · K-means Clustering"
...
```{r, eval=FALSE}
# Re-run perform_GSEA to get the full GSEA object (for gseaplot2)
gsea.res.obj = perform_GSEA(res = res, ref = hallmark, pvalueCutoff = 1)

# Significant pathways 
ids = gsea.res.obj@result %>% filter(p.adjust <= 0.05) %>% rownames()
```

```{r, eval=FALSE, fig.width=7, fig.height=6, out.width="60%"}
enrichplot::gseaplot2(gsea.res.obj, geneSetID = ids[1], title = ids[1])
enrichplot::gseaplot2(gsea.res.obj, geneSetID = ids[2], title = ids[2])
enrichplot::gseaplot2(gsea.res.obj, geneSetID = ids[3], title = ids[3])
enrichplot::gseaplot2(gsea.res.obj, geneSetID = ids[4], title = ids[4])
```
...
```{r, eval=T, fig.width=4,fig.height=5, out.width="40%"}
# Plot it 
...
# Multiple genes-violin plot across samples {.tabset}   
...
## Plot1
```{r,echo=T, eval=TRUE, fig.width=8, fig.height=5}
...
DEG1 %>% DEG_table() %>% 
  DT::datatable(extensions = "Buttons", 
                ...
                options = list(dom="Bfrtip",
                               scrollX=TRUE,
                               buttons=c("csv","excel"), pageLength=10))
```
...
## Select the signicant DEGs for the next step  
...
cbind(df3, (tpm %>% filter(rownames(.) %in% rownames(df6)))) %>% 
  DT::datatable(rownames = T, 
                caption = "Cluster information with TPM value", 
                width = "800px",
...
cbind(df4, (tpm %>% filter(rownames(.) %in% rownames(df6)))) %>% 
  DT::datatable(rownames = T, 
                caption = "Cluster information with TPM value", 
                width = "800px",
                options = list(scrollX=TRUE))
...
cbind(df5, (tpm %>% filter(rownames(.) %in% rownames(df6)))) %>% 
  DT::datatable(rownames = T, 
                caption = "Cluster information with TPM value", 
                width = "800px",
                options = list(scrollX=TRUE))
...
cbind(df6, (tpm %>% filter(rownames(.) %in% rownames(df6)))) %>% 
  DT::datatable(rownames = T, 
                caption = "Cluster information with TPM value", 
                width = "800px",
                options = list(scrollX=TRUE))
